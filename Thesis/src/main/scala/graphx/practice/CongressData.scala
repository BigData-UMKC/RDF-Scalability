...
object CongressData {
  ...
  def main(args: Array[String]) {
    ...
    val sc = new SparkContext(new SparkConf().setAppName("CongressData").setMaster("local[*]").set("spark.executor.memory", "3g"))
    ...
    val source = Source.fromFile("src/main/resources/inputData/samplecongress.nt", "iso-8859-1")
    val lines = source.getLines.toArray
    //printing all
    System.out.println("####################printing Triples as line############################")
    for (i <- 0 until lines.length){
      System.out.println("Triple "+i+": "+lines(i))
    }
    // When parsing the data we read, use this map to check whether each
    // URI has come up before.
    val vertexURIMap = new HashMap[String, Long];   //K V

    // Parse the data into triples.
    var triple = new Array[String](2)
    var nextVertexNum = 0L
    System.out.println("####################printing Triples after regex############################")
    for (i <- 0 until lines.length) {
      ...
      System.out.println("object "+i+": "+triple(i))
